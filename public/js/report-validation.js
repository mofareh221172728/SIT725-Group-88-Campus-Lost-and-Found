...
function validateTitle(value) {
    ...
    if (trimmed.length < TITLE_MIN_LEN) {
        return {
            valid: false,
            message: `Title must be at least ${TITLE_MIN_LEN} characters long.`
        };
    }
    ...
    if (trimmed.length > TITLE_MAX_LEN) {
        return {
            valid: false,
            message: `Title cannot exceed ${TITLE_MAX_LEN} characters.`
        };
    }
...
function validateCategory(value) {
    if (!value || value.trim() === '') {
        return {
            valid: false,
            message: 'Please select a category for the item.'
        };
    }
...
function validateCampus(value) {
    if (!value || value.trim() === '') {
        return {
            valid: false,
            message: 'Please select the relevant campus location.'
        };
    }
...
function validateBuilding(value) {
    ...
    if (!trimmed) {
        return {
            valid: false,
            message: 'Building location is required (e.g. Building B).'
        };
    }
...
function validateDate(value) {
    if (!value) {
        return {
            valid: false,
            message: 'Please select the relevant date.'
        };
    }
...
function validateHandoverMethod(value, reportType) {
    if (reportType !== 'found') {
        return {
            valid: true
        };
    ...
    if (!['email', 'dropoff'].includes(value)) {
        return {
            valid: false,
            message: 'Please select a handover method.'
        };
    }
...
function validateCollectionLocation(value, reportType, handoverMethod) {
    if (reportType !== 'found' || handoverMethod !== 'dropoff') {
        return {
            valid: true,
            sanitized: ''
        };
    ...
    if (!trimmed) {
        return {
            valid: false,
            message: 'Collection location is required for items that were dropped off.'
        };
    }
...
function validateDescription(value) {
    ...
    if (trimmed.length < DESC_MIN_LEN) {
        return {
            valid: false,
            message: `Description must be at least ${DESC_MIN_LEN} characters long.`
        };
    }
    ...
    if (trimmed.length > DESC_MAX_LEN) {
        return {
            valid: false,
            message: `Description cannot exceed ${DESC_MAX_LEN} characters.`
        };
    }
...
function validatePhotos(fileList) {
    ...
    if (fileList.length > MAX_PHOTOS) {
        return {
            valid: false,
            message: `You can upload a maximum of ${MAX_PHOTOS} photos.`
        };
    }
    ...
    for (let i = 0; i < fileList.length; i++) {
        ...
        if (!ALLOWED_MIME_TYPES.includes(file.type)) {
            return {
                valid: false,
                message: `File "${file.name}" is not supported. Use JPG, PNG, or WEBP.`
            };
        }
...
function validateReportForm(formElement) {
    const errors = [];
    const reportType = formElement.querySelector('#report-type')?.value;
    ...
    if (!campusRes.valid) {
        errors.push({
            element: campusSelect,
            message: campusRes.message
        });
    }
    ...
    if (!buildingRes.valid) {
        errors.push({
            element: buildingInput,
            message: buildingRes.message
        });
    }
    ...
    if (!photoRes.valid) {
        errors.push({
            element: photoInput,
            message: photoRes.message
        });
    }

    const roomInput = formElement.querySelector('#item-room');

    const handoverInput = formElement.querySelector(
        'input[name="handoverMethod"]:checked'
    );

    const firstHandoverInput = formElement.querySelector(
        'input[name="handoverMethod"]'
    );

    const handoverRes = validateHandoverMethod(
        handoverInput?.value,
        reportType
    );

    if (!handoverRes.valid) {
        errors.push({
            element: firstHandoverInput,
            message: handoverRes.message
        });
    }

    const collectionLocationInput = formElement.querySelector(
        '#collection-location'
    );

    const collectionLocationRes = validateCollectionLocation(
        collectionLocationInput?.value,
        reportType,
        handoverInput?.value
    );

    if (!collectionLocationRes.valid) {
        errors.push({
            element: collectionLocationInput,
            message: collectionLocationRes.message
        });
    }
    ...
    return {
        ...
        data: {
            ...
            handoverMethod: handoverRes.sanitized || null,
            collectionLocation: collectionLocationRes.sanitized || null
        }
    };
}
...
function validateSingleField(fieldElement) {
    ...
    } else if (id === 'collection-location') {
        const form = fieldElement.form;

        const reportType = form?.querySelector('#report-type')?.value;

        const handoverMethod = form?.querySelector(
            'input[name="handoverMethod"]:checked'
        )?.value;

        return validateCollectionLocation(
            fieldElement.value,
            reportType,
            handoverMethod
        );
...
if (typeof window !== 'undefined') {
    ...
    window.validateHandoverMethod = validateHandoverMethod;
