...
function buildModelFilter(dateField, { keyword, category, location, fromDate, toDate }) {
  ...
  if (location) {
    filter.campusLocation = new RegExp(escapeRegex(location), "i");
  }
  ...
  if (keyword) {
    ...
    return {
      $and: [filter, { $or: [{ title: regex }, { description: regex }] }],
    };
  }
...
const MODEL_CONFIG = {
  found: { model: FoundItem, dateField: "foundAt" },
  lost: { model: LostItem, dateField: "lostAt" },
};
...
async function getItems({
  ...
  const sortDirection = sort === "oldest" ? 1 : sort === "newest" ? -1 : null;

  const validTypes = type === "all"
    ? ["found", "lost"]
    : MODEL_CONFIG[type] ? [type] : [];

  if (validTypes.length === 0) {
    return isPaginated
      ? { items: [], total: 0, page: pageNumber, totalPages: 1 }
      : [];
  }

  const isSingle = validTypes.length === 1;

  const results = await Promise.all(
    validTypes.map(async (itemType) => {
      const { model, dateField } = MODEL_CONFIG[itemType];
      const filter = buildModelFilter(dateField, { keyword, category, location, fromDate, toDate });

      let query = model
        .find(filter)
        .select(`title description category campusLocation ${dateField} photos status`);

      if (sortDirection) {
        query = query.sort({ [dateField]: sortDirection });
      }

      if (isPaginated && isSingle) {
      ...
      return {
        count,
        items: docs.map((doc) => toCardItem(doc, itemType, dateField)),
      };
    })
  );
  ...
  if (!isSingle && sortDirection) {
    items.sort((a, b) => (sortDirection === 1 ? new Date(a.date) - new Date(b.date) : new Date(b.date) - new Date(a.date)));
  }
  ...
  if (isPaginated) {
    ...
    return { items, total, page: pageNumber, totalPages };
  }
